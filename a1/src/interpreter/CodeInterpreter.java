...
public class CodeInterpreter implements StatementVisitor, ExpTransform<Value> {
    ...
    public void visitMultiAssignNode(StatementNode.MultiAssignNode node) {
        beginExec("Multiple Assignment");
        List<StatementNode.AssignmentNode> assignments = node.getAssignments();
        HashMap<Value, Value> evaluatedValues = new HashMap<>();

        // Evaluate all left hand sides before assigning
        for (StatementNode.AssignmentNode assignment : assignments) {
            Value lValue = assignment.getVariable().evaluate(this);
            Value value = assignment.getExp().evaluate(this);
            evaluatedValues.put(lValue, value);
        }
        // Assign values after all evaluating
        for (Value lValue: evaluatedValues.keySet()) {
            Value value = evaluatedValues.get(lValue);
            assignValue(lValue, value);
        }
        endExec("Multiple Assignment");
    }

    /**
     * Execute code for a case branch statement, code is executed within visitCaseNode
     */
    public void visitBranchNode(StatementNode.BranchNode node) {
        beginExec("Branch");
        endExec("Branch");
    }

    /**
     * Execute code for a case node
     */
    public void visitCaseNode(StatementNode.CaseNode node) {
        beginExec("Case");
        ExpNode def = Type.optDereferenceExp(node.getConditionNode());
        Value condition = def.evaluate(this);
        List<StatementNode> branches = node.getCaseBranches();
        boolean branchExecuted = false;

        for (StatementNode branch : branches) {
            StatementNode.BranchNode branchAsNode = (StatementNode.BranchNode) branch;
            if (condition.getInteger() == branchAsNode.getConstantNode().getValue()) {
                branchAsNode.getStatementListNode().accept(this);
                branchExecuted = true;
                break;
            }
        }
        // If did not match branch then execute default if found
        if (!branchExecuted) {
            if (node.isDefaultExists()) {
                node.getDefaultNode().accept(this);
            } else {
                runtime("expression in case doesn't match any label",
                        node.getEndLoc(), currentFrame);
            }
        }
        endExec("Case");
    }
