 ...
 * Statement -> WhileStatement | IfStatement | CallStatement | MultiAssignment |
 ...
 * MultiAssignment -> Assignment { BAR Assignment }
 * Assignment -> LValue ASSIGN condition
 * CaseStatement -> KW_CASE Condition KW_OF { CaseBranch }
...
public class Parser {
    ...
    /**
     * Parse the non-terminal name rule, that starts with expected, in a
    ...
    private ExpNode parseRelCondition(TokenSet recoverSet) {
        return exp.parse("Condition", REL_CONDITION_START_SET, recoverSet,
                () -> {
    ...
    /**
     * Rule: Statement -> MultiAssignment | WhileStatement | IfStatement
    ...
    private StatementNode parseStatement(TokenSet recoverSet) {
        return stmt.parse("Statement", STATEMENT_START_SET, recoverSet,
                () -> {
                    ...
                    switch (tokens.getKind()) {
                        case IDENTIFIER:
                            return parseMultiAssignment(recoverSet);
                        case KW_WHILE:
    ...
    /**
     * Rule: MultiAssignment -> Assignment { BAR Assignment }
     */
    private StatementNode parseMultiAssignment(TokenSet recoverSet) {
        return stmt.parse("Assignment", LVALUE_START_SET, recoverSet,
                () -> {
                    Location loc = tokens.getLocation();
                    StatementNode.AssignmentNode assign =
                            parseAssignment(recoverSet.union(Token.BAR));
                    ArrayList<StatementNode.AssignmentNode> all =
                            new ArrayList<>();
                    all.add(assign);

                    /* If more assignments, parse them and add to list */
                    while (tokens.isMatch(Token.BAR)) {
                        tokens.match(Token.BAR);
                        StatementNode.AssignmentNode anotherAssign =
                                parseAssignment(recoverSet.union(Token.BAR));
                        all.add(anotherAssign);
                    }
                    return new StatementNode.MultiAssignNode(loc, all);
                });
    }
    ...
    /**
     * Set of tokens that may start a Case branch.
     */
    ...
    /**
     *  Rule: CaseStatement -> KW_CASE Condition KW_OF { CaseBranch }
     *            [ KW_DEFAULT StatementList ] KW_END
     */
    private StatementNode parseCaseStatement(TokenSet recoverSet) {
        return stmt.parse("Case Statement", Token.KW_CASE, recoverSet,
                () -> {
                    Location loc = tokens.getLocation();
                    tokens.match(Token.KW_CASE, CONDITION_START_SET);
                    ExpNode condition =
                            parseCondition(recoverSet.union(Token.KW_OF));
                    tokens.match(Token.KW_OF, CASE_BRANCH_START_SET);
                    /* List of StatementNodes, as may become an ErrorNode later. Casting done
                     * when type checking */
                    ArrayList<StatementNode> branches = new ArrayList<>();
                    while(tokens.isMatch(Token.KW_WHEN)) {
                        /* Add default in case last branch before default, add when in case
                         * non-last case branch */
                        StatementNode branch =
                                parseCaseBranch(recoverSet.union(Token.KW_DEFAULT).union(Token.KW_WHEN));
                        branches.add(branch);
                    }

                    if (tokens.isMatch(Token.KW_DEFAULT)) {
                        tokens.match(Token.KW_DEFAULT);
                        StatementNode defaultStatements =
                                parseStatementList(recoverSet.union(Token.KW_END));
                        Location endLoc = tokens.getLocation();
                        tokens.match(Token.KW_END);
                        return new StatementNode.CaseNode(loc, endLoc, condition, branches,
                                defaultStatements);
                    } else {
                        Location endLoc = tokens.getLocation();
                        tokens.match(Token.KW_END);
    ...
    private StatementNode parseCaseBranch(TokenSet recoverSet) {
        return stmt.parse("Case Branch", Token.KW_WHEN, recoverSet,
                () -> {
                    ...
                    ConstExp amount = parseConstant(recoverSet.union(Token.COLON));
                    tokens.match(Token.COLON, STATEMENT_START_SET);
                    StatementNode sl = parseStatementList(recoverSet.union(Token.KW_WHEN));
                    return new StatementNode.BranchNode(loc, amount, sl);
                });
    }
                ...
                () -> {
                    ...
                    if (proc == null) {
                        errors.fatal("Could not add main program to symbol table",
                                tokens.getLocation());
                        return null; // Doesn't get to this from fatal
                    }
