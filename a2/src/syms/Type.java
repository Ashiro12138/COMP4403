...
public abstract class Type {
     ...
     */
    public final static int NULL_ADDRESS = 0x80808080;
    /**
     ...
     */
    Type(Location loc, int space, boolean resolved) {
        this.loc = loc;
        this.space = space;
     ...
     */
    Type(Location loc, int space) {
        this(loc, space, false);
    }

    Type(Location loc, int space, boolean resolved, String name) {
    ...
    public static class ScalarType extends Type {
        ...
        ScalarType(Location loc, int size, int lower, int upper) {
            super(loc, size, true);
            this.lower = lower;
            this.upper = upper;
        }

        ScalarType(Location loc, String name, int size, int lower, int upper) {
            this(loc, size, lower, upper);
            this.name = name;
        }

        ScalarType(String name, int size, int lower, int upper) {
    ...
    public static class PointerType extends AddressType {
        ...
        public PointerType(Type baseType) {
            this(ErrorHandler.NO_LOCATION, baseType); // A pointer is an address
        }
        ...
        public boolean equals(Type other) {
            return other instanceof PointerType &&
                    ((PointerType) other).getBaseType() == this.getBaseType();
        }
        ...
        public static class RecordType extends Type {
            ...
            private final List<Field> fieldList; /* In order declared */
            private final SortedMap<String, Field> fields;

            public RecordType() {
                super(ErrorHandler.NO_LOCATION, 0); // Type size will need to be resolved later
